...
enum KeyStoreType {
  ...
  KeyStore getKeyStoreInstance() {
    try {
      ...
      throw new KeyException(
          "Unable to get key store instance of type "
              + this
              + ", because "
  ...
  KeyStore loadKeyStore(InputStream keyStoreStream, String keyStorePassword) {
    if (keyStoreStream == null) {
      throw new KeyException(
          "Failed to initialize key store, because the key store stream is null. Please specify a stream with data.");
    } else {
      ...
      try {
        ...
        throw new KeyException(
            "Unable to load key store instance of type "
                + this
                + ", because "
