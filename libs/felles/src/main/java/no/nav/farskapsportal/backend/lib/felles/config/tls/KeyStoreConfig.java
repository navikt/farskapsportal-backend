...
import no.nav.farskapsportal.backend.lib.felles.exception.CertificateException;
...
public class KeyStoreConfig {
  ...
  public Certificate[] getCertificateChain() {
    try {
      ...
      throw new KeyException("Failed to retrieve certificate chain from key store. Is key store initialized?", var2);
    }
  }
  ...
  public X509Certificate getCertificate() {
    ...
    try {
      ...
      throw new CertificateException("Failed to retrieve certificate from key store. Is key store initialized?", var3);
    }
  ...
  public PrivateKey getPrivateKey() {
    try {
      ...
      throw new KeyException("Failed to retrieve private key from key store. Is key store initialized?", var2);
    } catch (NoSuchAlgorithmException var3) {
      throw new KeyException("Failed to retrieve private key from key store. Verify that the key is supported on the platform.", var3);
    } catch (UnrecoverableKeyException var4) {
      throw new KeyException("Failed to retrieve private key from key store. Verify that the password is correct.", var4);
    }
  }
  ...
  public static KeyStoreConfig fromOrganizationCertificate(InputStream organizationCertificateStream, String privatekeyPassword) {
    ...
    try {
      ...
      throw new KeyException("Could not retrieve aliases from the key store, because " + var5.getClass().getSimpleName() + ": '" + var5.getMessage() + "'. Are you sure this is an organization certificate in PKCS12 format?", var5);
    }
  ...
  private void verifyCorrectAliasCasing(Certificate certificate) {
    try {
      ...
      throw new CertificateException("Unable to get certificate alias based on certificate. This should never happen, as we just read the certificate from the same keystore.", var3);
    }
  }
}
