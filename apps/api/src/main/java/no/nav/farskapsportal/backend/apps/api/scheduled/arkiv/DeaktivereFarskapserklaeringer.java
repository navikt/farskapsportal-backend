...
public class DeaktivereFarskapserklaeringer {
  ...
  @Scheduled(
      cron = "${farskapsportal.asynkron.egenskaper.arkiv.deaktiveringsrate}",
      zone = "Europe/Oslo")
  ...
  private void deaktivereFarskapserklaeringerMedUtgaatteSigneringsoppdrag() {
    ...
    var eldsteGyldigeDatoForSigneringsoppdrag = LocalDate.now().minusDays(levetid);
    var utloepstidspunkt = eldsteGyldigeDatoForSigneringsoppdrag.atStartOfDay();

    var idTilFarskapserklaeringerMedUtgaatteSigneringsoppdrag =
    ...
    if (idTilFarskapserklaeringerMedUtgaatteSigneringsoppdrag.size() > 0) {
      log.info(
          "Fant {} ikke-ferdigstilte farskapserklæringer med signeringsoppdrag eldre enn {} dager. Deaktiverer disse.",
          idTilFarskapserklaeringerMedUtgaatteSigneringsoppdrag.size(),
          levetid);
    } else {
      log.info(
          "Fant ingen ikke-ferdigstilte farskapserklæringer med signeringsoppdrag eldre enn {} dager",
          levetid);
      return;
    }
    ...
    for (int farskapserklaeringsid : idTilFarskapserklaeringerMedUtgaatteSigneringsoppdrag) {
      ...
      var aktiveOppgaver =
          persistenceService.henteAktiveOppgaverTilForelderIFarskapserklaering(
              farskapserklaeringsid, farskapserklaering.getFar());
      log.info(
          "Fant {} utløpte signeringsoppgaver knyttet til farskapserklæring med id {}.",
          aktiveOppgaver.size(),
          farskapserklaeringsid);
      for (Oppgavebestilling oppgave : aktiveOppgaver) {
        log.info(
            "Sletter utgått signeringsoppgave for far (id {}) i farskapserklæring (id {})",
      ...
      if (aktiveOppgaver.size() > 0) {
        ...
        brukernotifikasjonConsumer.varsleMorOmUtgaattOppgaveForSignering(
  ...
  private void deaktivereFarskapserklaeringerSomErSendtTilSkatt() {
    ...
    var idTilFarskapserklaeringerSomSkalDeaktiveres =
        persistenceService.henteIdTilOversendteFarskapserklaeringerSomErKlarForDeaktivering(
            tidspunktOversendtFoer);

    if (idTilFarskapserklaeringerSomSkalDeaktiveres.size() > 0) {
      log.info(
          "Fant {} ferdigstilte farskapserklæringer som har blitt overført til Skatt for minst {} dager siden, og er klare for deaktivering.",
      ...
      log.info(
          "{} ferdigstilte farskapserklæringer ble i denne omgang deaktivert",
          antallErklaeringerSomBleDeaktivert);
    } else {
      log.info(
          "Fant ingen ferdigstilte farskapserklæringer som har blitt overført til Skatt for minst {} dager siden, og er klare for deaktivering",
          levetid);
    }
  }
  ...
  private void deaktivereFarskapserklaeringerSomManglerMorsSignatur() {
    ...
    var morSendtTilSigneringFoer = LocalDateTime.now().minusDays(levetid);

    var idTilFarskapserklaeringerSomSkalDeaktiveres =
        persistenceService.henteIdTilFarskapserklaeringerSomManglerMorsSignatur(
            morSendtTilSigneringFoer);
    if (idTilFarskapserklaeringerSomSkalDeaktiveres.size() > 0) {
      log.info(
          "Fant {} farskapserklæringer som ikke er signert av mor etter {} dager -> deaktiverer disse.",
          idTilFarskapserklaeringerSomSkalDeaktiveres.size(),
          levetid);
      for (int farskapserklaeringsid : idTilFarskapserklaeringerSomSkalDeaktiveres) {
        antallErklaeringerSomBleDeaktivert =
      ...
      log.info(
          "{} usignerte farskapserklæringer ble i denne omgang deaktivert",
          antallErklaeringerSomBleDeaktivert);
    } else {
      log.info(
          "Fant ingen farskapserklæringer som ikke er signert av mor etter {} dager -> ingen å deaktivere.",
          levetid);
    }
  }
}
