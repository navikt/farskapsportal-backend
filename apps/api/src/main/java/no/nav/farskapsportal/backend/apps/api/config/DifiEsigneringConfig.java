...
public class DifiEsigneringConfig {
  ...
  public DifiEsigneringConfig(
    ...
    this.miljoe =
        navClusterName.equals(NavClusterName.PROD.getClusterName())
            ? NavClusterName.PROD.toString()
            : NavClusterName.TEST.toString();
  }
  ...
  public ClientConfiguration clientConfiguration(KeyStoreConfig keyStoreConfig) {
    var serviceEnvironment =
        miljoe.equals(NavClusterName.TEST.toString())
            ? ServiceEnvironment.STAGING
            : ServiceEnvironment.PRODUCTION;
    log.info(
        "Kobler opp mot Postens {}-milljø for esignering med service-uri {}.",
    ...
    return ClientConfiguration.builder(keyStoreConfig)
        ...
        .timeoutsForDocumentDownloads(
            t -> {
              t.responseArrivalTimeout(ofMillis(15000));
              t.connectionRequestTimeout(ofMillis(20000));
            })
