...
import no.nav.farskapsportal.backend.libs.felles.exception.PDFConsumerException;
...
public class PdfGeneratorConsumer {
  ...
  private static final Map<Elementnavn, String> elementnavnTilEngelsk =
      Map.ofEntries(
  ...
  private static final Map<Elementnavn, String> nynorskeElementnavn =
      Map.ofEntries(
  ...
  private static final Map<Tekst, String> tekstBokmaal =
      Map.of(
  ...
  private static final Map<Tekst, String> tekstNynorsk =
      Map.of(
          Tekst.FOEDESTED, "Fødestad",
          Tekst.NAVN, "Namn",
          Tekst.OPPLYSNINGER_OM_BARNET, "Opplysningar om barnet");

  private static final Map<Tekst, String> tekstEngelsk =
      Map.of(
  ...
  public byte[] genererePdf(
      BarnDto barnMedDetaljer,
      ForelderDto morMedDetaljer,
      ForelderDto farMedDetaljer,
      Skriftspraak skriftspraak) {

    log.info("Oppretter dokument for farskapserklæring på {}", skriftspraak);

    var html =
        byggeHtmlstrengFraMal(
            STI_TIL_PDF_TEMPLATE, skriftspraak, barnMedDetaljer, morMedDetaljer, farMedDetaljer);
    try (final ByteArrayOutputStream pdfStream = new ByteArrayOutputStream()) {
      ...
      org.jsoup.nodes.Document jsoupDoc =
          Jsoup.parse(htmlSomStroem, "UTF-8", "pdf-template/template.html");
      Document doc = new W3CDom().fromJsoup(jsoupDoc);
      var builder = new PdfRendererBuilder();

      try (InputStream colorProfile =
          PdfGeneratorConsumer.class.getResourceAsStream(
              "/pdf-template/ISOcoated_v2_300_bas.ICC")) {
      ...
      try (InputStream fontStream =
          PdfGeneratorConsumer.class.getResourceAsStream("/pdf-template/Arial.ttf")) {
      ...
      throw new PDFConsumerException(Feilkode.OPPRETTE_PDF_FEILET, ioe);
    }
  }

  private void leggeTilDataBarn(
      org.jsoup.nodes.Element barnElement, BarnDto barnDto, Skriftspraak skriftspraak) {
    if (barnDto.getFoedselsnummer() != null) {
      barnElement.getElementsByClass(henteElementnavn(Elementnavn.UFOEDT, skriftspraak)).remove();
      var beskrivelse =
          barnElement.getElementsByClass(henteElementnavn(Elementnavn.BESKRIVELSE, skriftspraak));
      beskrivelse.first().text(tekstvelger(Tekst.OPPLYSNINGER_OM_BARNET, skriftspraak));
      var foedselsdato =
          barnElement.getElementsByClass(henteElementnavn(Elementnavn.FOEDSELSDATO, skriftspraak));
      foedselsdato
          .first()
          .text(
              tekstvelger(Tekst.FOEDSELSDATO, skriftspraak)
                  + ": "
                  + barnDto.getFoedselsdato().format(DateTimeFormatter.ofPattern("dd.MM.yyyy")));
      var foedselsnummer =
          barnElement.getElementById(henteElementnavn(Elementnavn.FOEDSELSNUMMER, skriftspraak));
      foedselsnummer.text(
          tekstvelger(Tekst.FOEDSELSNUMMER, skriftspraak) + ": " + barnDto.getFoedselsnummer());
      if (barnDto.getFoedested() != null) {
        var foedested =
            barnElement.getElementById(henteElementnavn(Elementnavn.FOEDESTED, skriftspraak));
      ...
      var termindato =
          barnElement.getElementById(henteElementnavn(Elementnavn.TERMINDATO, skriftspraak));
      termindato.text(
          tekstvelger(Tekst.TERMINDATO, skriftspraak)
              + ": "
              + barnDto.getTermindato().format(DateTimeFormatter.ofPattern("dd.MM.yyyy")));
    }
  }

  private void leggeTilDataForelder(
      org.jsoup.nodes.Element forelderelement, ForelderDto forelderDto, Skriftspraak skriftspraak) {
    var navn = forelderelement.getElementsByClass(henteElementnavn(Elementnavn.NAVN, skriftspraak));

    navn.first()
        .text(
            tekstvelger(Tekst.NAVN, skriftspraak) + ": " + forelderDto.getNavn().sammensattNavn());

    var foedselsdato =
        forelderelement.getElementsByClass(
            henteElementnavn(Elementnavn.FOEDSELSDATO, skriftspraak));
    foedselsdato
        .first()
        .text(
            tekstvelger(Tekst.FOEDSELSDATO, skriftspraak)
                + ": "
                + forelderDto.getFoedselsdato().format(DateTimeFormatter.ofPattern("dd.MM.yyyy")));

    var foedselsnummer =
        forelderelement.getElementsByClass(
            henteElementnavn(Elementnavn.FOEDSELSNUMMER, skriftspraak));
    foedselsnummer
        .first()
        .text(
            tekstvelger(Tekst.FOEDSELSNUMMER, skriftspraak)
                + ": "
                + forelderDto.getFoedselsnummer());
  }

  private String byggeHtmlstrengFraMal(
      String pdfmal, Skriftspraak skriftspraak, BarnDto barn, ForelderDto mor, ForelderDto far) {
    try {
      var input =
          new ClassPathResource(pdfmal + skriftspraak.toString().toLowerCase() + ".html")
              .getInputStream();
      var document = Jsoup.parse(input, "UTF-8", "");

      // Legge til informasjon om barn
      leggeTilDataBarn(
          document.getElementById(henteElementnavn(Elementnavn.BARN, skriftspraak)),
          barn,
          skriftspraak);
      // Legge til informasjon om mor
      leggeTilDataForelder(
          document.getElementById(henteElementnavn(Elementnavn.MOR, skriftspraak)),
          mor,
          skriftspraak);
      // Legge til informasjon om far
      leggeTilDataForelder(
          document.getElementById(henteElementnavn(Elementnavn.FAR, skriftspraak)),
          far,
          skriftspraak);

      // jsoup fjerner tagslutt for <link> og <meta> - legger på manuelt ettersom dette er påkrevd
      // av PDFBOX
