package no.nav.farskapsportal.backend.asynkron.scheduled;
...
public class SletteOppgave {
  ...
  public void sletteUtloepteSigneringsoppgaver() {
    ...
    for (Farskapserklaering farskapserklaering : farskapserklaeringerSomVenterPaaFar) {
      if (farskapsportalAsynkronEgenskaper.getBrukernotifikasjon().isSkruddPaa() && farskapserklaering.getDokument().getSigneringsinformasjonMor()
          .getSigneringstidspunkt().toLocalDate()
          .isBefore(LocalDate.now().minusDays(farskapsportalAsynkronEgenskaper.getBrukernotifikasjon().getSynlighetOppgaveAntallDager() - 1))) {
